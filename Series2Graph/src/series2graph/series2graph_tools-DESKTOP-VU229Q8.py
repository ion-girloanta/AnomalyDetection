...
def build_phase_space(T,latent,m,rate=1):
	...
	current_seq = [0]*m
	first = True
	for i in range(int((len(T) - m)/rate)):  #Why len(T) - m - latent ? range(0, 877)
		tmp = []
		it_rate = i*rate     # Why ?
		if first:
			first = False
			for j in range(m - latent): # why m - latent ?   range(0, 50)
				tmp.append(sum(x for x in T[it_rate+j:it_rate+j+latent+1]))   # latent = 50    (49, 74)
			tmp_glob.append(tmp)
			current_seq = tmp
		else:
			tmp = current_seq[1:]
			tmp.append(sum(x for x in T[it_rate+m-latent-1:it_rate+m]))    # (51, 76)  Missed one value
			tmp_glob.append(tmp)
			current_seq = tmp
