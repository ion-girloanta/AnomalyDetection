...
#  for i in range(0,len(self.graph["edge_in_time"])-int(query_length))       range(0, 825) - 
...
class Series2Graph():
	...
	def score(self,query_length):
		...
		for i in range(0,len(self.graph["edge_in_time"])-int(query_length)):   # query_length = 100
	...
	def plot_graph(self):
		...
		pos = nx.nx_agraph.graphviz_layout(nx.Graph(self.graph["list_edge"]),prog="dot") # prog="fdp", Options depend on GraphViz version but may include: ‘dot’, ‘twopi’, ‘fdp’, ‘sfdp’, ‘circo’
